...
1. **扬声器音频播放验证**：测试 MAX98357A 能否正确播放指定 PCM 音频流。

### 按键功能验证

2. **按键事件验证**：验证消抖逻辑在实际按键抖动下的稳定性，确认定时器中断优先级配置无冲突。

### 云端处理验证

3. **上行音频流式识别及文字回传验证**：测试设备端录音流式上传 → 云端 ASR 流式识别 → 识别结果流式回传的完整流程，确认识别文本能实时正确显示在 LCD 上。
4. **WebUI 后端端到端验证**：验证 LLM 对话交互功能，确认每轮对话的用户录音文件、TTS 合成音频文件及上下行文字均已正确在后端隔离存储；确认 WebUI 前端能以文字气泡形式展示对话内容，并在每条记录提供对应录音或合成语音的在线播放控制，还原真实对话场景，建立完整的上下文复盘能力。
5. **下行流式回传验证**：验证 LLM 回复文本与 TTS 音频能否并行流式下发到设备，设备能否通过帧类型标识将两路数据正确分流——文本流实时更新 LCD 对话气泡，音频流实时送入 MAX98357A 外放，两路同步无卡顿。

### 显示逻辑验证与修复

7. **状态机与 LCD 显示联调**：逐状态验证屏幕 UI 是否与系统状态同步刷新，重点检查状态切换时的界面清屏与重绘逻辑，排查残影或刷新不完整问题。
8. **录音动效**：验证按住 K1 录音期间屏幕动效能否正常驱动，松开后动效停止与状态提示切换是否及时。
9. **对话气泡与滚动显示**：测试长对话内容的自动换行、气泡高度计算与上下滚动翻页逻辑，验证滚动条比例显示是否正确。开发板翻阅历史消息时与服务器的交互方式（按需分页拉取 vs. 一次性加载缓存）及本地缓存策略尚未确定，需结合 64KB 内存限制设计。
10. **历史会话列表显示**：验证历史列表条目渲染、光标高亮切换与会话载入后的屏幕刷新流程。

### 完整流程闭环

11. **端到端完整对话流程测试**：在以上各模块均验证通过后，执行完整的 按键录音 → 音频上传 → 云端识别并下发文本 → LLM 对话 → TTS 合成 → 文本和音频下发 → 扬声器播放及屏幕更新 的全链路测试，确认各环节无阻塞与数据丢失。
12. **WebUI 对话记录与音频联动**：验证每轮对话的用户录音文件、TTS 合成音频文件及上下行文字均已正确写入后端存储；确认 WebUI 前端能以文字气泡形式展示对话内容，并在每条记录提供对应录音或合成语音的在线播放控制，还原真实对话场景，建立完整的上下文复盘能力。
